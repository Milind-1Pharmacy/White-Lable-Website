...
function SavingsRow({ row }: { row: { name: string; cat: string; brandPrice: number; ourPrice: number; pct: number; color?: string } }) {
  const [ref, seen] = useReveal();
    ...
    <div className="sv__row" ref={ref}>
      ...
      <div className="sv__row-ladder">
        <div className="sv__price sv__price--brand">
          ...
          <span className="sv__price-amt">₹<AnimNum value={row.brandPrice} /></span>
        </div>
        ...
        <div className="sv__price sv__price--ours">
          ...
          <span className="sv__price-amt">₹<AnimNum value={row.ourPrice} /></span>
        </div>
      </div>
      <div className="sv__row-bar">
        <div className="sv__row-bar-track">
          <div className="sv__row-bar-fill" style={{ width: seen ? row.pct + "%" : 0, background: color, transition: "width 1.2s cubic-bezier(.4,0,.2,1)" }} />
        </div>
      </div>
    </div>
  );
...
export function Savings({ data }: SavingsProps) {
  ...
  return (
    <section className="section section--cream">
      <div className="wrap">
        <div className="between section-head" style={{ marginBottom: 40, alignItems: "end", flexWrap: "wrap" }}>
          <div>
            ...
            <h2 className="h-display h-2" style={{ marginTop: 14, maxWidth: 760 }}>
              ...
              <span className="serif-it" style={{ color: "var(--accent)" }}>brand options</span> in medicines.
            </h2>
          </div>
        ...
        <div className="sv__ledger">
          <div className="sv__ledger-head">
            ...
            <span className="sv__ledger-tag">
              <span className="serif-it" style={{ fontSize: 30, color: "var(--accent)", lineHeight: 1 }}>59%</span>
          ...
          <div className="sv__ledger-foot">
            <span className="mono" style={{ fontSize: 11, letterSpacing: ".14em" }}>
              * Indicative for a 30-day pack. Actual savings vary by composition, strength &amp; location.
            </span>
          </div>
        </div>
            ...
            {!playing ? (
              <>
                ...
                <div className="sv__video-overlay">
                  <span className="imgbox__tag" style={{ background: "rgba(255,255,255,.92)", color: "var(--ink)" }}>
