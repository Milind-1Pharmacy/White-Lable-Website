...
type AppStripProps = { data: AppStripSectionData };

export function AppStrip({ data }: AppStripProps) {
  return (
          ...
          <div>
            <h2 className="appstrip__h">
              ...
              <span className="serif-it" style={{ color: "var(--accent)" }}>
                {data.heading ? data.heading.replace("Download the ", "").replace(" app", "") : "UrMedz"}
              </span>{" "}
              app
            </h2>
            <p className="appstrip__body">{data.descriptor ?? "Authentic medicines · licensed pharmacists · doorstep delivery"}</p>
          </div>
        </div>
        <div className="appstrip__badges">
          {data.appStoreUrl && (
            <a className="store-badge store-badge--apple" href={data.appStoreUrl} target="_blank" rel="noreferrer">
          ...
          {data.googlePlayUrl && (
            <a className="store-badge store-badge--google" href={data.googlePlayUrl} target="_blank" rel="noreferrer">
